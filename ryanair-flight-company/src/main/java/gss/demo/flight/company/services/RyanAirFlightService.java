...
import gss.demo.flight.company.repositories.RyanAirFlightsRepository;
import lombok.extern.slf4j.Slf4j;

@Slf4j
@Service
public class RyanAirFlightService {

	private final RyanAirFlightsRepository airRepository;
	private final AirportRepository airportRepository;

	public RyanAirFlightService(RyanAirFlightsRepository airRepository, AirportRepository airportRepository) {
	...
	public void addFlight(FlightModel model) {
		RyanAirFlight flight = new RyanAirFlight();
		if(model == null) {
		...
		RyanAirFlight saved = airRepository.save(flight);
		log.info("Saved flight with id: " + saved.getId());
	}
	
	private FlightModel entityToModel(Flight entity) {
	...
	private List<FlightModel> toModelList(List<RyanAirFlight> entityList) {
